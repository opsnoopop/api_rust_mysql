...
struct User {
  user_id: i32, // int i32, Bigint u64
  username: String,
  email: String,
}
...
async fn main() -> anyhow::Result<()> {
  ...
  // แนะนำให้ default เป็นชื่อ service ใน compose ("database")
  let db_host = env_or("DB_HOST", "container_mysql");
  ...
  let dsn = format!(
      "mysql://{user}:{pass}@{host}:{port}/{name}?ssl-mode=DISABLED&charset=utf8mb4",
...
async fn create_user(
  ...
  if payload.username.trim().is_empty() || payload.email.trim().is_empty() {
      return (StatusCode::BAD_REQUEST, Json(ErrMsg { error: "username and email are required", detail: None })).into_response();
  }

  let fut = sqlx::query("INSERT INTO users (username, email) VALUES (?, ?)")
...
async fn get_user(
  State(state): State<AppState>,
  Path(id): Path<i32>, // int <i32>, Bigint <u64>
) -> Response {
